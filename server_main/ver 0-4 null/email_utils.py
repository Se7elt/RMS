...
from database_utils import register_device_in_db, create_metrics_table, insert_metrics_to_db, insert_system_log
...
def process_metrics_email(connection, ip_address, email_body):
    ...
    try:
        ...
        for block in metric_blocks:
            ...
            # Убедимся, что метрики корректно извлечены
            if metrics:
                # Сохраняем метрики в базу данных
                insert_metrics_to_db(connection, ip_address, metrics)

                # Добавляем метрики в хранилище для проверки пределов
                if ip_address not in metric_storage:
                    metric_storage[ip_address] = {"metrics": [], "counter": 0}

                metric_storage[ip_address]["metrics"].append(metrics)
                metric_storage[ip_address]["counter"] += 1

                # Проверяем пороги после каждых 10 сообщений
                if metric_storage[ip_address]["counter"] == 10:
                    check_thresholds_and_log(connection, ip_address, metric_storage[ip_address]["metrics"])
                    # Сбрасываем счетчик и метрики для IP
                    metric_storage[ip_address] = {"metrics": [], "counter": 0}

    except Exception as e:
        logging.error(f"Ошибка при обработке письма с метриками для IP {ip_address}: {e}")

def check_thresholds_and_log(connection, ip_address, metrics_list):
    """Проверка превышения порогов и запись логов в system_logs."""
    thresholds = load_thresholds()

    # Вычисляем средние значения
    avg_cpu_load = sum(m["cpu_load_proc"] for m in metrics_list) / len(metrics_list)
    avg_ram_usage = sum(m["ram_usage_proc"] for m in metrics_list) / len(metrics_list)
    avg_temperature = sum(m["temperatures_C"] for m in metrics_list) / len(metrics_list)

    # Логи, если превышены пороговые значения
    if avg_cpu_load > thresholds["CPU_LOAD_LIMIT"]:
        insert_system_log(
            connection,
            ip_address,
            avg_cpu_load,
            avg_ram_usage,
            avg_temperature,
            f"CPU load exceeded threshold: {avg_cpu_load:.2f}%"
        )

    if avg_ram_usage > thresholds["RAM_USAGE_LIMIT"]:
        insert_system_log(
            connection,
            ip_address,
            avg_cpu_load,
            avg_ram_usage,
            avg_temperature,
            f"RAM usage exceeded threshold: {avg_ram_usage:.2f}%"
        )

    if avg_temperature > thresholds["TEMPERATURE_LIMIT"]:
        insert_system_log(
            connection,
            ip_address,
            avg_cpu_load,
            avg_ram_usage,
            avg_temperature,
            f"Temperature exceeded threshold: {avg_temperature:.2f}°C"
        )
