...
from database_utils import connect_db, ensure_device_info_table, register_device_in_db, create_metrics_table, create_system_logs_table, insert_system_log
...
# Функция для вставки записи в system_logs
def insert_system_log(connection, ip_address, cpu_load, ram_usage, temperature, log_message):
    try:
        cursor = connection.cursor()
        cursor.execute("""
            INSERT INTO system_logs (ip_address, cpu_load, ram_usage, temperature, log_message)
            VALUES (%s, %s, %s, %s, %s);
...
if __name__ == "__main__":
    ...
    # Создание таблицы system_logs, если она не существует
    create_system_logs_table(db_connection)
